...
def assemble_baseline_command(cfg: DictConfig) -> List[str]:
    ...
    if cfg.method in ["diffdock", "fabind", "dynamicbind", "neuralplexer", "rfaa", "vina"]:
        # NOTE: When running RoseTTAFold-All-Atom, the `RFAA` Conda environment must be activated instead of the `MULTICOM_ligand` environment
        ...
        return f"python3 multicom_ligand/models/{cfg.method}_inference.py dataset={cfg.dataset} repeat_index={cfg.repeat_index} max_num_inputs={cfg.max_num_inputs}{vina_postfix}{cuda_device_postfix}{rfaa_postfix}".split()
    else:
