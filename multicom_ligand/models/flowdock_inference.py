    ...
    if cfg.input_ligand is not None and cfg.input_receptor is not None:
        ...
        try:
            subprocess_args = [
                ...
                os.path.join(str(cfg.flowdock_exec_dir), "multicom_ligand", "sample.py"),
        ...
        if auxiliary_estimation_only:
            ...
            # install integer rank values from sample IDs
            combined_auxiliary_estimation_df["rank"] = combined_auxiliary_estimation_df[
                "sample_id"
            ].apply(lambda x: int(x.split("_rank")[1]))
