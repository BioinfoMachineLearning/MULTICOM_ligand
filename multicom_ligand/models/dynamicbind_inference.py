...
from multicom_ligand.utils.utils import find_ligand_files, find_protein_files
        ...
        try:
            subprocess.run(
                [
                    ...
                    "--no_relax",  # NOTE: must be set to `True` within `MULTICOM_ligand` since method-native relaxation is not supported
                    "--paper",  # NOTE: must be set to `True` within `MULTICOM_ligand` since only the paper weights are available
                ],
                check=True,
            )  # nosec
