...
def write_scripts(
    ...
    :param pocket_only_baseline: Whether to provide only the protein pocket as a baseline
        experiment.
    :param pdb_filepath: Optional path to the PDB structure file.
    ...
    if pdb_filepath is not None and ligand_smiles is not None:
        input_id = (
            "_".join(os.path.splitext(os.path.basename(pdb_filepath))[0].split("_")[:2])
            if input_id is None
            else input_id
        )
        # parse amino acid or nucleic acid chains
        chain_sequence_list = [
            seq
            for (seq, _) in extract_sequences_from_macromolecule_structure_file(pdb_filepath)
            if len(seq) > 0
        ]
        output_dir = os.path.join(output_scripts_path, input_id)
        for chain_index, sequence in enumerate(chain_sequence_list, start=1):
        ...
        for smiles_string, pdb_id in smiles_and_pdb_id_list:
            ...
            if casp_dataset_requested:
                pdb_filepath = os.path.join(input_data_dir, f"{pdb_id}_lig.pdb")
                ligand_filepaths = [
                    create_sdf_file_from_smiles(
                ...
                if pocket_only_baseline:
                    ...
                    if dataset == "ensemble":
                        ensemble_pdb_id = f"{pdb_id[:2]}000" if "L" == pdb_id[0] else pdb_id
                        pdb_filepath = os.path.join(
                            os.path.dirname(input_data_dir),
                            "ensemble_proteins",
                            f"{ensemble_pdb_id}.pdb",
                        )
                    else:
                        pdb_filepath = os.path.join(
                ...
                if dataset == "dockgen":
                    ...
                    ligand_filepaths = [
                        create_sdf_file_from_smiles(
                            smiles, os.path.join(output_dir, f"{ensemble_pdb_id}_{i}.sdf")
                        )
                        for i, smiles in enumerate(smiles_string.split(":"), start=1)
                    ]
                else:
            ...
            if not casp_dataset_requested and dataset not in ["dockgen", "ensemble"]:
                for ligand_filepath in ligand_filepaths:
                    shutil.copy(ligand_filepath, output_dir)
...
def main(cfg: DictConfig):
    ...
    if cfg.protein_filepath is not None and cfg.ligand_smiles is None:
        write_scripts(
            ...
            pdb_filepath=cfg.protein_filepath,
            ligand_smiles=cfg.ligand_smiles,
            input_id=cfg.input_id,
        )
    else:
        if cfg.dataset == "ensemble":
            assert cfg.input_csv_filepath is not None and os.path.exists(
                cfg.input_csv_filepath
            ), f"Invalid input CSV file path: {os.path.exists(cfg.input_csv_filepath)}."
            csv_df = pd.read_csv(cfg.input_csv_filepath)
            smiles_and_pdb_id_list = [
                (row["ligand_smiles"], row["name"]) for _, row in csv_df.iterrows()
            ]
        else:
