...
from beartype.typing import Any, List, Literal, Optional, Set, Tuple, Union
from Bio import PDB
...
@beartype
def extract_sequences_from_macromolecule_structure_file(
    pdb_filepath: Union[str, Path], structure: Optional[Structure] = None
) -> Tuple[List[str], List[Literal["DNA", "RNA", "Protein"]]]:
    """Extract the chain sequences from a macromolecule structure file.

    :param pdb_filepath: Path to the PDB structure file.
    :param structure: Optional BioPython structure object to use instead.
    :return: A list of macromolecule sequences and their corresponding molecule chain types (e.g., `Protein`).
    """
    biopython_parser = PDBParser()
    try:
        if structure is None:
            structure = biopython_parser.get_structure("random_id", pdb_filepath)
        structure = structure[0]
    except Exception as e:
        logger.error(f"Due to exception {e}, could not parse macromolecule {pdb_filepath}.")
        return None
    ...
    for chain in structure:
        ...
        for residue in chain:
            if residue.get_resname() == "HOH":
                continue
            if residue.get_resname() in DNA_THREE_TO_ONE:
                seq += DNA_THREE_TO_ONE[residue.get_resname()]
                sequence_types.append("DNA")
                continue
            if residue.get_resname() in RNA_THREE_TO_ONE:
                seq += RNA_THREE_TO_ONE[residue.get_resname()]
            ...
            if (
                ...
                try:
                    seq += PROTEIN_THREE_TO_ONE[residue.get_resname()]
                    sequence_types.append("Protein")
                except Exception as e:
                    seq += "-"
                    logger.error(
                        f"Due to exception {e}, encountered unknown amino acid {residue.get_resname()} in the macromolecule {pdb_filepath}. Replacing it with a dash (i.e., `-`)."
                    )
        sequences.append(seq)
    return sequences, sequence_types
