...
from src.utils.inspect_ode_samplers import clamp_tensor

rootutils.setup_root(__file__, indicator=".project-root", pythonpath=True)

from src.data.components.mol_features import collate_numpy_samples
from src.data.components.physical import get_vdw_radii_array, get_vdw_radii_array_uff
from src.models.components.cpm import resolve_pl_contact_stack, resolve_protein_encoder
from src.models.components.esdm import (
...
from src.utils.frame_utils import apply_similarity_transform
...
class FlowDock(torch.nn.Module):
    ...
    def run_score_head(
        self, batch: MODEL_BATCH, embedding_iter_id: Union[int, str], **kwargs: Dict[str, Any]
    ) -> MODEL_BATCH:
        """Run the score head.
        ...
        return self.score_head(
            batch, frozen_prot=self.global_cfg.frozen_protein_backbone, **kwargs
        )
    ...
    def run_affinity_head(self, batch: MODEL_BATCH, **kwargs: Dict[str, Any]) -> MODEL_BATCH:
        ...
        """
        aff_out = self.affinity_head(batch, frozen_prot=False, **kwargs)
